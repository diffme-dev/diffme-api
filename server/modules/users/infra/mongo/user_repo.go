...
func (m *UserRepo) FindByAuthProviderId(authProviderId string) (snapshot *domain.User, err error) {

	var snapshotDoc UserModel

	err = m.DB.Collection(modelName).Find(bson.M{
		"auth.provider_user_id": authProviderId,
	}).Query.One(&snapshotDoc)
